...
{
    return [[self alloc] initWithQueue:pipeline];
}
...
- (void)setQueue:(NSArray *)queue
{
    ...
    self.requestBuildBlock = ^(MFWHttpDataTask *task)
    {
        for (id <HttpRequestBuildProtocol> builder in weak_self.queue)
        {
            if ([builder conformsToProtocol:@protocol(HttpRequestBuildProtocol)] && builder.requestBuildBlock != nil)
            {
